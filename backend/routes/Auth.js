...
Auth.post("/mobileAPI/otp-verify", otpTokenVerification, async (req, res) => {
  ...
  if (!otpPattern.test(otp)) {
    return res
      .status(400)
      .json({ message: "Please enter a valid six-digit OTP" });
  }
  ...
  try {
    ...
    for (const currentUser of users) {
      if (currentUser.role === "student") {
        ...
        const studentToken = jwt.sign(studentData, JWT_SECRET, {
          expiresIn: "360m",
        });
        oneData.push({
          role: "student",
        ...
        const teacherToken = jwt.sign(teacherDetails, JWT_SECRET, {
          expiresIn: "360m",
        });
        oneData.push({
          role: teacher.adminAccess ? "admin-teacher" : "teacher",
        ...
        const adminToken = jwt.sign(adminDetails, JWT_SECRET, {
          expiresIn: "360m",
        });
