...
ManageBannerImages.get('/mobileAPI/bannerImages', completeLogin, async (req, res) => {
    try {
        ...
        const completeData = await prisma.bannerImages.findMany({
            where: {
                school_id,
            },
        });
...
ManageBannerImages.delete('/api/bannerImages/:id', AdminAuth('banner Images'), async (req, res) => {
    ...
    try {
        const bannerDetails = await prisma.bannerImages.findUnique({
            where: {id: bannerID},
        });
        ...
        if (bannerDetails) {
            ...
            res.send(404).json({message: 'Image not found'});
        }
