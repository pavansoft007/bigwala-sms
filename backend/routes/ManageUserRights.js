...
ManageUserRights.post("/api/roles", AdminAuth("roles"), async (req, res) => {
  try {
    ...
    if (!role_name || !permissions) {
      return res
        .status(400)
        .json({ message: "role_name and permissions are required." });
    }
...
ManageUserRights.get("/api/roles", AdminAuth("roles"), async (req, res) => {
  ...
  try {
    const roles = await Role.findAll({
      where: {
        school_id,
        role_name: { [Op.ne]: "admin" }, // used OP to exclude the admin role using sequelize
      },
    });
    ...
    try {
      ...
      if (isReferenced > 0) {
        return res
          ...
          .json({
            message:
              "Role cannot be deleted as it is referenced in other records",
          });
      }
...
ManageUserRights.get("/api/users", AdminAuth("roles"), async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ one: req["sessionData"]["admin_id"], data: users });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
...
ManageUserRights.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await User.destroy({
        where: { original_id: user.admin_id, role: "admin" },
      });
      await user.destroy();
