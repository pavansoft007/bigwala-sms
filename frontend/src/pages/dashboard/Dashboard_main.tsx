...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            await axiosInstance.get<DashboardData>('/api/main-dashboard').then((res) => {
                const formattedStats = [
                    {
                        title: "Total Students",
                        value: res.data.total_students.toString(),
                        icon: <Users size={24}/>
                    },
                    {
                        title: "Total Staff",
                        value: res.data.total_teachers.toString(),
                        icon: <Users size={24}/>
                    },
                    {
                        title: "Fee Collection",
                        value: `₹${formatIndianNumber(res.data.total_collection)}`,
                        icon: <CreditCard size={24}/>
                    },
                    {
                        title: "Attendance",
                        value: `${parseFloat(res.data.attendance_percentage).toFixed(2)}%`,
                        icon: <Clock size={24}/>
                    }
                ];
